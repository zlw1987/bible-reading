...
<body class = "w3-light-grey">
            ...
            <ul class="w3-ul w3-hoverable" style="width:100%" >
                <?php foreach ($all_comments as $gc): ?>
                    <li class="w3-card w3-round-xlarge w3-white" style="margin-bottom:5px">
                        ...
                        <?php if ((int) $gc['user_id'] === $userid): ?>
                            <form class="w3-right" action="delete_comment_en.php" method="post">
                                <?php echo csrf_field(); ?>
                                <input type="hidden" name="plan" value="<?php echo $s_plan_id; ?>">
                                <input type="hidden" name="id" value="<?php echo (int) $gc['id']; ?>">
                                <button type="submit" class="w3-button w3-padding-small">Delete</button>
                            </form>
                        <?php else: ?>
                            <a href="#" onclick="myReply(<?php echo h(json_encode($gc['fname'])); ?>, <?php echo h(json_encode($gc['lname'])); ?>, <?php echo (int) $gc['id']; ?>)" class="w3-right">Reply</a>
                        <?php endif; ?>
                        </div>
                        <div class="w3-padding">
                            <?php foreach (($all_reply[(int) $gc['id']] ?? array()) as $gr): ?>
                                ...
                                <?php else: ?>
                                    <a href="#" onclick="myReply(<?php echo h(json_encode($gr['fname'])); ?>, <?php echo h(json_encode($gr['lname'])); ?>, <?php echo (int) $gc['id']; ?>)" class="w3-right">Reply</a>
                                <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                        </div>
                ...
                <script>
                    function myReply(a,b,c) {
                        ...
                        document.getElementById('myTextarea').value = "Reply "+a+" "+b+": ";
    ...
    <div class = "w3-white w3-card-4 w3-bottom">
        ...
        <form action="insert_comment_en.php?plan=<?php echo $s_plan_id; ?>" id="comment" method="post">
            <?php echo csrf_field(); ?>
            ...
            <div id="note" class="note w3-display-topleft w3-padding-large" onclick = "myComment()">
                <font color="#777">&nbsp;Share today's devotional thoughts...</font>
            </div>
            <script>
                function myComment() {
                    document.getElementById('note').style.display='none';
            ...
            <a class="w3-button w3-small w3-round w3-black" style="margin: 10px 0px 0px 0px" href="<?php echo h($back_url); ?>">Back</a>
            <input class = "w3-right" style="margin: 14px 0px 0px 0px" type="submit" value = "Submit" onclick = "this.disabled=true; this.value='Sending…'; this.form.submit();">
        </form>
        </div>
    </div>
</body>
